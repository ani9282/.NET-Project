...
public partial class admin_Teacher_Registration : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            SqlConnection con = new SqlConnection(connectionString);
            con.Open();
            SqlCommand cmd = new SqlCommand("Select Distinct(Subject_Name) from CET_tblSubject", con);
            cmd.Connection = con;
            var obj = cmd.ExecuteReader();
            while(obj.Read())
            {
                DDL_Subject.Items.Add(obj.GetString(obj.GetOrdinal("Subject_Name")));
            }
            obj.Dispose();
        }
        catch (Exception ex)
